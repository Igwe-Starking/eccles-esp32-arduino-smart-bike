...
ECCLES_API {
    ...
    class AudioOutput {
        ...
        //audio clip pool this class and all its features must be called in a single thread
        //we decided to use xQueue to send clips instead of pool
        //static audioClip clips[MAX_CLIP_SIZE]; //max 10 audios in the pool
        ...
        //returns the audio buffer for filling must call release buffer if you later changed your mind about playing this buffer
        //audioClip* getBuffer(); we are no longer polling pools

        //release the audio buffer, this is only necessary if you don't call play
        //void releaseBuffer();
        ...
        //reconfigure the audio system with the given config
