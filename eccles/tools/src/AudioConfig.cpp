    ...
    */
    ...
    e_boolean AudioConfig::open(e_string path){
        ...
        if(!st){
            ...
            cout<<"failed to open ecclesConfig.txt from the following dir:"<<path<<endl;
            return false;
        }
    ...
    vector<e_uint8> AudioConfig::configureRate(vector<e_uint8> in,wav_header_t& header){
        ...
        for(e_uint64 i = 0;i < len;i+= bz){
            ...
            //bound check
            if(index >= sz - bz){
                int_p = Resampler::scaleUp(Resampler::getSample(in, config.depth, sz - bz), config.depth);
            } else {
                e_double sv  = Resampler::scaleUp(Resampler::getSample(in, config.depth, index),      config.depth); //get the sample at current index
                e_double sv1 = Resampler::scaleUp(Resampler::getSample(in, config.depth, index + bz), config.depth); //get the neighbouring sample

                //interpolate the two
                int_p = sv * (1.0 - frac) + sv1 * frac;
            }

            e_int ot = Resampler::scaleDown(int_p, config.depth);
            Resampler::writeSample(out, config.depth, ot, (e_uint32)i);
        }
        return out;
    }
