...
ECCLES_API {
    ...
    FileSystem::FileSystem(){
        ...
        //issue it started crashing on our live test so we default to lazy mounting the Littlefs system on load
    ...
    FileStatus FileSystem::load(e_string path,e_string mode){
        ...
        //we ran into littleFs bug where the system is failing to mount so we add true so littleFs formats itself
        if(!mounted) status = (mounted = LittleFS.begin(true)) ? FileStatus::SUCCESSFUL : FileStatus::MOUNT_FAILED;
        ...
        //be usable on littlefs system

        e_string pt_m = nullptr; //we use this to avoid modifying the original string
        e_char temp[32]; //holds the temporary modification data
        ...
        if(path[0] == '/'){
            ...
            temp[0] = '/';
            memcpy(temp + 1,path,strlen(path)+1); //string size + null terminator
            //temp[strlen(path)+1] = '\0'; //we add null terminator
            pt_m = temp;
        }
        ...
        //now we open our file for operation
        file = LittleFS.open(pt_m,mode);

        if(!file){
            //something must be wrong we check if the file even exists
            if(!LittleFS.exists(pt_m)) return (status = FileStatus::FILE_DOESNT_EXIST);
    ...
    FileStatus FileSystem::readAll(e_uint8 * buffer,e_uint32 len){
        ...
        file.seek(0); //resets file cursor
    ...
    FileStatus FileSystem::write(e_uint8* buffer,e_uint32 len){
        ...
        if(eccles_compareString(mode,"a")){
            ...
            file.seek(0); //resets file cursor
            file.seek(f_ptr); //we sets cursor to the write position
    ...
    FileStatus FileSystem::unload(){
        ...
        file.close();
        file = File(); //resets the file object to default
        return FileStatus::SUCCESSFUL;
    }
};