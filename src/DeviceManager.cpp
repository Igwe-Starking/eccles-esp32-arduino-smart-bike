...
ECCLES_API {
    ...
    */
    ...
    //creating all devices from pin table, we only need this for now, we may need something like
    //createDevicesFromArray() in the future.
    /* this is commented out because it causes local variable and pointers that points to garbage so we default to 
    global initialization instead;

    void DeviceManager::createFromPins(){
      ECCLES_LOG_LINE("creating all devices from pin table");

      //we start by initializing all pins and set there modes,this is done in Pins for now
      Pins::initializeAll();

      //creating IO devices, we needed to create them first.
      //the order must match with DeviceID order.

      auto length = sizeof(Pins::OUTPUT_PINS);

      for(int i = 0;i< length;i++){
        //we create device by looping through Device ids so that each device match perfectly with its id
        if(Pins::OUTPUT_PINS[i] == Pins::IGNITION_CONTROL_PIN){
          //ignition is a special device
          devices[i] = DigitalOutputAnalogFeedbackDevice(Pins::OUTPUT_PINS[i],Pins::FEEDBACK_PINS[i],false,"ignition");
          deviceCount++;
          continue;
        }
        devices[i] = DigitalIODevice(Pins::OUTPUT_PINS[i],Pins::FEEDBACK_PINS[i],true,Pins::getPinName(Pins::OUTPUT_PINS[i]));
        deviceCount++;
      }

      analogOffset = length; //analog devices starts here.

      //creating analog/sensor devices

      sensorCount = sizeof(Pins::SENSOR_PINS);
      for(int i = 0;i < sensorCount; i ++){
        if(Pins::SENSOR_PINS[i] == Pins::SHOCK_SENSOR_PIN){
          //shock sensor is a special serial device.
          devices[i + analogOffset] = SerialInputDevice(Pins::SENSOR_PINS[i]);
          deviceCount++;
          continue;
        }
        devices[i + analogOffset] = AnalogInputDevice(Pins::SENSOR_PINS[i]);
        deviceCount++;
      }
      if(deviceCount != (analogOffset + sensorCount)){
        ECCLES_LOG_LINE("error creating device");
        return;
      }
      ECCLES_LOG_LINE("devices created successfully");
    ...
    void LiveData::monitor(){
      ...
      if(ldm_d == 0){
        ldm_d = (millis() + 500); //5ms
        return;
      }

      if(ldm_d > millis()) return; //time not reached

      //we update the global state for things that needs it
      //writeGlobal();
