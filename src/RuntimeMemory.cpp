...
ECCLES_API {
    ...
    */
    ...
    void freeBuffer(e_uint8* buffer){
        ...
        //clear the whole buffer -- disabled per request to avoid the cost of
        //zeroing on every free (10s of blocks * up to 256 bytes each). NOTE:
        //this means a freshly allocated buffer may still contain the previous
        //owner's data until the new owner writes into it; callers that rely
        //on a freshly-zeroed buffer must memset it themselves after e_malloc
