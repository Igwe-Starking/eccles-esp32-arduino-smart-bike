  ...
  void Executor::onReceive(Command& com){
    ...
    if(com.duration == 0 && com.interval ==0){
      ...
      if(com.duration <= t){
        ...
        if(com.interval > 0){
          com.duration = millis() + (com.delay * 1000);
          com.interval--;
          return;
        }
      } else {
    ...
    */
    ...
    while(word != nullptr){
      ...
      switch (input){
        ...
        case H_GREATER : {
          //greater can only be used in conditions or monitors
          if(conditionRequest){
            ...
            if(svl){
              ...
              if(com->condition.target != DeviceID::UNKNOWN_DEVICE){
                //conditions target is already set and since greater does't need action we disable condition arg checking
                com->condition.exists = true;
            ...
            if(m_data == nullptr){
              ...
              if(!m_data) break;
              memset(m_data,0,9);
              if(mst) m_data[0] = atoi(mst);
              com->size = 9;
              com->data = m_data;
            } else {
              //if m_data is already created then the target must have been set we check though
              if(mst) m_data[0] = atoi(mst);
              com->size = 9;
              com->data = m_data;
        ...
        case H_LESSER : {
          //less than can only be used in conditions or monitors
          if(conditionRequest){
            e_string svl = strtok(nullptr," "); //get the next data
            if(svl){
              //we can forgive condition anomalies
        ...
        case H_FOR : {
          ...
          e_uint8 dl = atoi(fms);
          com->delay = dl; //if you don't specify a valid digit here your delay stays at 0
          com->duration = millis() + (com->delay * 1000);
          break;
        }
        case H_TIMES : {
      ...
      if(d_id != DeviceID::UNKNOWN_DEVICE){ //here we go,thats a target
        if(conditionRequest){
              ...
              if(com->condition.action != CommandAction::NO_OP || com->condition.value > 0){
                //conditions action is already set  we disable condition arg checking
                com->condition.exists = true;
                conditionRequest = false;
              }
            ...
            if(m_data == nullptr){
              ...
              memset(m_data,0,9);
              m_data[3] = static_cast<e_uint8>(d_id);
              com->size = 9;
              com->data = m_data;
            } else {
              //if m_data is already created then the target must have been set we check though
              ...
              com->data = m_data;
              com->dataType = MNT_MGC;
              if((m_data[2] != 0) || (m_data[0] > 0)) monitorRequest = false; //stop searching for monitor data
            }
      ...
      if(c_at != CommandAction::NO_OP){
         if(conditionRequest){
            ...
            if(m_data == nullptr){
              ...
              memset(m_data,0,9);
              m_data[2] = static_cast<e_uint8>(c_at);
              com->size = 9;
              com->data = m_data;
            } else {
              //if m_data is already created then the target must have been set we check though
              ...
              com->data = m_data;
              com->dataType = MNT_MGC;
              if((m_data[3] != 0) || (m_data[0] > 0)) monitorRequest = false; //stop searching for monitor data
            }
